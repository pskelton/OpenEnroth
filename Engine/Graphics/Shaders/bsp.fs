...
void main() {
    ...
    if (olayer.x == 0) {
        ...
        if (olayer.y == 0){
            if (vsAttrib & 0x3C00){
                fragcol = texture(textureArray0, vec3(texcoords.x/4.0,texcoords.y/4.0,0));
            } else {
                fragcol = texture(textureArray0, vec3(texcoords.x/4.0,texcoords.y/4.0,waterframe));//watercol;
            }
        }
    }
